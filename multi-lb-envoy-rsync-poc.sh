...
# Number of Envoy gateway pods behind the single LoadBalancer Service.
ENVOY_REPLICAS="${ENVOY_REPLICAS:-3}"
...
echo "ENVOY_REPLICAS=$ENVOY_REPLICAS"
...
EOF_ENVOY
  ...
  #   - Envoy Deployment
  #   - one LB service exposing many TCP ports
...
YAML
  ...
  if ! k_src apply -f "$SOURCE_YAML"; then
    ...
    nl -ba "$SOURCE_YAML" | sed -n '1,260p'
    exit 1
  fi
  ...
  k_dest rollout status deployment/"$GW_DEPLOY" --timeout="$WAIT_TIMEOUT"
